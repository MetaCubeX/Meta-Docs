...
!!! note "Различия между стеками протоколов"
    ...
    * `mips` использует IP-стек, разработанный mihomo
...
Указывает IPv6-адрес для tun

!!! note
    * В настоящее время программа проверяет, имеют ли другие сетевые карты в системе IPv6 при запуске. Если нет, эта функция будет отключена. Чтобы принудительно использовать свой IPv6-адрес для tun, пожалуйста, вручную установите системную переменную среды `SKIP_SYSTEM_IPV6_CHECK=1`.
    * Это вступит в силу только в том случае, если в конфигурации верхнего уровня также установлено значение `ipv6` равное `true`.
