...
Обязательное поле. Имя провайдера (например, `provider1`). Должно быть уникальным. Рекомендуется, чтобы оно не совпадало с именами [групп прокси (Proxy Groups)](../proxy-groups/index.md#name).
...
См. общие поля [name-cert-verify](../proxies/tls.md#name-cert-verify).
