...
!!! note
    Обратите внимание, что используемые здесь подстановочные знаки отличаются от [подстановочных знаков формата Clash](../../handbook/syntax.md#domain-wildcards) в других местах конфигурационного файла.
...
Сопоставляет целевой [диапазон портов](../../handbook/syntax.md#port-ranges) запроса.

### SRC-PORT

Сопоставляет исходный [диапазон портов](../../handbook/syntax.md#port-ranges) запроса.

### IN-PORT

Сопоставляет [входящий порт](../inbound/listeners/index.md#port), допускает [диапазоны портов](../../handbook/syntax.md#port-ranges).
...
Сопоставление по подстановочным знакам для пути процесса, поддерживает только `*` и `?`.
...
Сопоставление по подстановочным знакам для имени процесса, поддерживает только `*` и `?`. На платформе Android также может сопоставлять имена пакетов.
...
### GEOIP вместо GEOSITE

`GEOIP` сопоставляет только **код страны** (двухбуквенный ISO: `RU`, `CN`, `US`). Правила вида `GEOIP,discord`, `GEOIP,chatgpt`, `GEOIP,antifilter` **не работают** как ожидается — для сервисов используйте `GEOSITE`, `DOMAIN`, `RULE-SET` или `IP-CIDR`/`IP-ASN`.

### DOMAIN с IP-адресом

`DOMAIN` ожидает **имя хоста** (FQDN), а не IPv4/IPv6 и не CIDR. Для адресов и подсетей используйте `IP-CIDR` / `IP-CIDR6` (при необходимости с `no-resolve`).

### fake-ip-filter-mode: rule + IP-CIDR

В `fake-ip-filter` при `fake-ip-filter-mode: rule` допустимы **только доменные** селекторы (`DOMAIN*`, `GEOSITE`, `RULE-SET`, `MATCH`). `IP-CIDR`, `GEOIP` и `IP-ASN` вызовут ошибку парсинга. 
