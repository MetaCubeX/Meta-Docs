...
!!! note
    Как и другие протоколы прокси, Tailscale является необязательным узлом прокси. Трафик будет проходить через Tailscale только в том случае, если вы направите его с помощью правил, групп политик или других методов маршрутизации.

    Более того, все типы прокси начинают подключаться только при первом успешном подключении (т.е. они не запускаются синхронно с miHoMo). Поэтому таймаут при первой попытке доступа к узлу Tailscale является нормальным явлением; пожалуйста, повторяйте попытки, пока Tailscale не загрузится успешно.
