...
!!! note
    ...
    Кроме того, proxy любого типа начинает подключаться только при первом запросе, который на него совпал (то есть не запускается вместе с Mihomo). Поэтому таймаут при первой попытке доступа к узлу Tailscale является нормальным явлением; повторяйте попытки, пока Tailscale не загрузится успешно.
