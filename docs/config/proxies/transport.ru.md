...
Максимальное количество прокси-запросов, которые могут существовать одновременно в рамках каждого базового соединения. После достижения этого значения будет установлено новое соединение для обработки большего количества прокси-запросов.

### xhttp-opts.reuse-settings.max-concurrency

Максимальное количество одновременных соединений. До достижения этого значения каждый новый прокси-запрос будет открывать новое соединение. После этого будут повторно использоваться существующие соединения. Это значение конфликтует с `max-connections`; можно выбрать только одно из них.
