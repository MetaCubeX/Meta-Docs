...
Опционально, алгоритм аутентификации данных, поддерживаются `MD5` /`SHA1` / `SHA256`/ `SHA384` / `SHA512`, по умолчанию `SHA256`. При использовании AEAD-шифров (например, GCM или CHACHA20-POLY1305) параметр `auth` игнорируется.
