...
Опционально, метод шифрования, поддерживаются `AES-128-GCM` / `AES-192-GCM` / `AES-256-GCM` / `AES-128-CBC` / `AES-192-CBC` / `AES-256-CBC` / `CHACHA20-POLY1305`, по умолчанию `AES-128-GCM`. Значение `AES-CBC` будет обрабатываться как `AES-128-CBC`.
