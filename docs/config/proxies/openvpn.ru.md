...
Количество секунд, в течение которых старый ключ данных (data key) сохраняется после смены ключа (rekey); по умолчанию — 3600. Явное значение 0 означает немедленное истечение срока действия. Должно быть согласовано с серверным параметром --tran-window.
