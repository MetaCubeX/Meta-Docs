...
!!! tip "Для Keenetic"
    На роутерах Keenetic рекомендуется `xudp` — он критически важен для стабильной работы голосовых каналов Discord и онлайн-игр без потерь датаграмм.
...
!!! warning "XTLS Vision + smux"
    При использовании `flow: xtls-rprx-vision` мультиплексирование (`smux.enabled`) должно быть отключено — XTLS Vision несовместим с mux.
